...
2) We'll switch to a [live coding Rmd](https://github.com/aedobbyn/nyc-fires/blob/master/live_code.md) which hopefully won't totally break `r emo::ji("crossed_fingers")` to run a `drake` pipeline
...
[GNU Make](https://www.gnu.org/software/make/) is a tool that uses a file called a Makefile to specify **dependencies** in a pipeline. 
...
But, `knitr` is a reporting tool, not a pipelining tool.

- You can't summon values from the `cache` in an interactive session

- If you're outsourcing preprocessing steps to R scripts outside the `Rmd`, we haven't solved any of the usual dependency issues from before

- `Rmd`s quickly get big and unweildy for serious pipelines

---

## Where are targets stored?
...
<p align="left" style="padding-right: 15%; padding-bottom: 30%;">

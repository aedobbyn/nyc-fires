...
library(knitr)
opts_chunk$set(echo = TRUE, eval = TRUE, collapse = TRUE)
...
- if that function is called again with the *same* set of arguments, we pull the return value from the cache rather than recomputing it --> saves time `r emo::ji("thumbsup")`
...
What if a function upstream of the memoised function changes? We could get the wrong answer. `r emo::ji("thumbsdown")`
...
`drake` stores a dependency graph (`igraph` object) of the plan along with a bunch of other things in a `config` list.
...
We can analyze these tweets to get some info on where and when fires happen in NYC.
...
We will **send address** and **receive lat long**
...
or join on some `ggplot2::map_data`
...
We don't want to re-run the main pipeline every time we change a downstream analysis step.
...
`drake` will save us **`r round(n_mins, digits = 0)` minutes** of our lives *and* we'll still rest assured that our downstream targets have the most up to date data.

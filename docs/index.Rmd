...
`drake` knows that`results` depends on `cleaned_data` and `analyze_my()`
...
If all our downstream mapping and counting analysis depends on these trips to and from Twitter and Google, we don't want to re-run that portion of the pipeline for `r round(n_mins, digits = 0)` each time we tweak the analysis unless something changes.
...
Okay on to part 2 after this message from our sponsor.

...
- if that funciton is called again with the *same* set of arguments, we pull the return value from the cache rather than recomputing it --> saves time
...
But now we redefine `add` so that it subtracts `b` from `a`...
```{r}
add <- function(a, b) {
  a - b
}

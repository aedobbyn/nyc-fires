...
burner_plan <-
  drake_plan(
    ...
    seed_burn = get_tweets(
      user = burner_handle,
      input = here(file_in("data/raw/burn.csv"))
    ),
    
    full_burn = get_tweets(
        tbl = seed_burn,
        user = burner_handle,
        output_path = here(file_out("data/raw/burn.csv")) # Outputs to burner_path
      )
  )
```
...
The `file_in` and `file_out` syntax allow `drake` to recognize and track the file. That lets us see when the targets associated with the file (both upstream and downstream of it) are invalidated.
...
burner_plan_2 <-
  drake_plan(
    ...
    seed_burn = get_tweets(
      user = burner_handle,
      input_path = here(file_in(burner_path))
    ),
    ...
    full_burn = target(
      command = get_tweets(
        tbl = seed_burn,
        user = burner_handle,
        output_path = here(file_out(burner_path)) # Outputs to burner_path
      ),
      trigger = trigger(
...
plan <-
  drake_plan(
    seed_fires = get_tweets( 
      n_tweets_seed = 2,
      max_id = old_tweet_id,
      input_path = here(file_in(fire_path))
    ), 
    fires = target(
      command = get_tweets(
        tbl = seed_fires,
        n_tweets_reup = 3,
        output_path = here(file_out(fire_path))
      ),
      trigger = trigger(
        condition = TRUE # Always look for new tweets
      )
    ...
    plot = plot_fire_sums(dat, nyc,
                          output_path =
                                    here(file_out("plots/fire_sums.png")))
  )
```

...
Let's define a path to output the tweets we collect to.
...
```{r}
burner_plan # Take a look at the dataframe that represents our plan
burner_config <- drake_config(burner_plan)
clean() # Remove targets if they were already built
vis_drake_graph(burner_config) # Both seed_burn and full_burn should be outdated
...
```{r}
clean()
vis_drake_graph(burner_config) # Targets are outdated now
# This time since we saved the previous output of full_burn to a file, we read that in for our seed_burn
make(burner_plan) 
vis_drake_graph(burner_config) # And now everything is up to date again
```
...
```{r, eval=FALSE}
clean()
burner_plan_2 # Notice our new trigger column
...
make(burner_plan_2)
loadd(seed_burn) # Load the results of seed_burn and full_burn into our environment
loadd(full_burn)
expect_identical(seed_burn, full_burn) # These should be the same since there were no tweets posted between when we made seed_burn and full_burn

outdated(burner_config_2) # full_burn will always be outdated because of our trigger
vis_drake_graph(burner_config_2)
```
...
make(burner_plan_2) # seed_burn will be read in from the file, which reflects the state of the world before this tweet. Then full_burn will incorporate the new tweet.
vis_drake_graph(burner_config_2) # seed_burn is up to date, and full_burn will always look outdated

loadd(seed_burn)
loadd(full_burn)
expect_gt(nrow(full_burn), nrow(seed_burn)) # We should have one extra row in full_burn than in seed_burn
...
Cool, so we now know that we can successfully trigger the re-building of `full_burn` every time and save it as the latest state of the world to a file. 

Every time new tweets arrive, that file will be updated at the end of the `make()`.
...
Let's modify the code to a function called by `pull_addresses`. Note that `drake` recognizes that targets that incorporate this function at some point in the pipeline are out of date, even though this function isn't called in the plan directly.
...
We'll change the early return value if a borough can't be detected from `NA` to just "NY":
